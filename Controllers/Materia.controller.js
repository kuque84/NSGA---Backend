...
exports.listaPag = async (req, res) => {
  ...
  try {
    const registros = await db.Materia.findAndCountAll({
      include: [
        {
          model: db.Curso,
          as: "Curso",
      ...
      where: {
        [Op.or]: [
          { nombre: { [Op.like]: `%${text}%` } },
          literal(
            `EXISTS (SELECT 1 FROM cursos AS Curso WHERE Curso.id_curso = Materia.id_curso AND Curso.nombre LIKE '%${text}%')`
          ),
          literal(
            `EXISTS (SELECT 1 FROM cursos AS Curso INNER JOIN planes AS Plan ON Plan.id_plan = Curso.id_plan WHERE Curso.id_curso = Materia.id_curso AND (Plan.codigo LIKE '%${text}%' OR Plan.descripcion LIKE '%${text}%'))`
          ),
        ],
      },
      limit,
